...
static const struct arg args[] = {
	...
	{ datetime,  "^C0^^B6^ 󰔟 ^f8^^d^^C6^%s^f2^",      "%F %T",    "0",          1  },
	{ ram_perc,  "^C0^^B2^   ^f8^^d^^C2^%s%%",   NULL,       "1",          1  },
	{ cpu_perc,  "^C0^^B5^   ^f8^^d^^C5^%s%%",   NULL,       "2",          1  },
	{ disk_free, "^C0^^B4^ 󱛟 ^f8^^d^^C4^%s free", "/",        "3",          60 },
	{ run_exec,  "^C0^^B1^ %s ",        "~/slstatus-sus/scripts/funcs/volume", "4", 1 },
	{ keymap,    " ^C0^^B3^  ^f8^^d^^C3^%s",	    NULL, 	    "5",          1 },
};
